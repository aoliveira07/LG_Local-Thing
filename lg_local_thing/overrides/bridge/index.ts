// Smart House bridge diagnostics and explicit registration, 2026-09-15. GPL v2.
...
class BridgedDevice {
    ...
    reconnectNow() {
        ...
        if (U instanceof Thinq1Device && D instanceof T1Downstream) {
            ...
            this.connection.on('data', (payload) => withCommandSource(D.id, 'lg', () => D.send(payload)))
        } else if (U instanceof Thinq2Device && D instanceof T2Downstream) {
            this.connection = new Thinq2Connection(U)
            this.connection.on('data', (payload) => withCommandSource(D.id, 'lg', () => D.send_packet(payload)))
        } else {
...
export class Bridge extends TypedEmitter<BridgeEvents> {
    ...
    #start(dev: AnyDevice) {
        if (!this.state.getCredentials()) return
    ...
    registrationRequired(id: string) {
        const dev = this.manager.allDevices[id]
        return !!dev && !this.bridgedDevices.has(id) && !this.loadSavedDevice(dev)
    }

    async enable(id: string, devType?: string, statusCallback?: StatusCallback, confirmRegistration = false) {
        if (this.bridgedDevices.has(id)) return true
        ...
        const saved = this.loadSavedDevice(dev)
        if (saved) {
            this.bridgedDevices.set(dev.id, new BridgedDevice(saved, dev))
            this.emit('started', dev.id)
            return true
        }
        if (!confirmRegistration) throw new Error('Confirme o recadastro deste aparelho na conta LG antes de ativar o bridge.')
    ...
    logout() {
        ...
        for (const id of [...this.bridgedDevices.keys()]) this.#stop(id)
