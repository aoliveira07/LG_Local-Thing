...
import { friendlyNames, FriendlyIdentity } from '@/util/friendly-names'
...
function componentSlug(value: string): string {
    const slug = value
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
...
/*
 * Smart House entity identity policy
 *
 * The user-facing name can change at any time. The entityBase is created only
 * when the friendly name is configured for the first time and is never changed
 * afterwards.
 *
 * Example:
 *
 ...
 *   default_entity_id = climate.ar_casal
 *
 * After changing the display name to "Ar Suite":
 *   device.name        = "Ar Suite"
 *   default_entity_id  = climate.ar_casal
 *   unique_id          = unchanged
 */
function applyFriendlyIdentity(config: DeviceDiscovery, identity: FriendlyIdentity): DeviceDiscovery {
    const componentEntries = Object.entries(config.components)

    const primaryComponentKey = componentEntries.find(([, component]) => component.name === null)?.[0]

    const components = Object.fromEntries(
        componentEntries.map(([key, component]) => {
            const entityObjectId =
                key === primaryComponentKey
                    ? identity.entityBase
                    : `${identity.entityBase}_${componentSlug(key)}`

            return [
                key,
                {
                    ...component,
                    default_entity_id: `${component.platform}.${entityObjectId}`,
                },
            ]
        }),
    )
...
export class Connection extends TypedEmitter<ConnectionEvents> {
    ...
    publishConfig(id: string, config: DeviceDiscovery) {
        /*
         * Do not create Home Assistant entities until this device has received
         * a Smart House friendly name.
         *
         * This is intentional: the first Discovery message determines the
         * preferred entity_id. A new "Ar Casal" therefore starts directly as
         * climate.ar_casal instead of first becoming a generic LG entity.
         */
        const identity = friendlyNames.get(id)

        if (!identity) {
            log('publish', id, 'Discovery skipped: friendly name not configured')
            return
        }
        ...
        const namedConfig = applyFriendlyIdentity(config, identity)
        const configPayload = JSON.stringify(recursiveReplace(namedConfig, replacements))
